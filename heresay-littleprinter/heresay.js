...
exports.sample = function(req, res){
	...
	request.get('http://heresay.org.uk/api/get_recent_updates.php?borough=Lambeth%20Borough%20Council', function (error, response, body) {
...
exports.edition = function(req, res){
	...
	if (req.param('test') == "true") {
		...
		//request.get('http://www.heresay.org.uk/api/get_recent_favourites.php', function (error, response, body) {
		//request.get('http://heresay.org.uk/api/get_recent_updates.php?lat=51.45967697948443&lng=-0.12342899999998735&tags=', function (error, response, body) {
		request.get('http://heresay.org.uk/api/get_near_by.php?postcode=' + postcode_param, function (error, response, body) {
			//request.get('http://heresay.org.uk/api/get_recent_updates.php?borough=Islington%20Borough%20Council', function (error, response, body) {
			if (!error && response.statusCode == 200) {
				console.log(body) // Output response to console
				jsonObject = JSON.parse(body);
